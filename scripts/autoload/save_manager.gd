...
func load_game() -> bool:
	...
	for u in upgrades:
		GameManager.unlocked_upgrades.append(u)
	GameManager.starting_cards = data.get("starting_cards", [])
	grid_save_data = data.get("grid", {})
	shop_slots_save = data.get("shop_slots", [])
	GameManager.points_changed.emit(GameManager.points)
	return true
...
func deserialize_grid(data: Dictionary) -> GridData:
	var g := GridData.new(data.get("width", 18), data.get("height", 8))
